    ...
    computed: {
        filteredContacts() {
            return this.contacts.filter(contact => {
                ...
                const matchesStatus =
                    this.statusFilter === 'all' || contact.status === this.statusFilter
                return matchesSearch && matchesStatus
            })
        },
        statusLabel() {
            return this.statusFilter === 'all' ? 'All Status' : this.statusFilter.charAt(0).toUpperCase() + this.statusFilter.slice(1)
        }
    },
    watch: {
            ...
            <div class="flex gap-2 flex-col sm:flex-row w-full">
                ...
                <div class="relative">
                    <button @click="showStatusDropdown = !showStatusDropdown"
                        ...
                        <Funnel />
                        {{ statusLabel }}
                        <ChevronDown class="text-(--emerald-medium)" />
                    </button>
                    <div v-show="showStatusDropdown"
                        class="absolute right-0 mt-1 w-40 bg-white rounded-xl border border-(--emerald-light) shadow-lg z-10 overflow-hidden">
                        <button v-for="option in ['all', 'active', 'pending', 'inactive']" :key="option"
                            @click="statusFilter = option; showStatusDropdown = false"
                            :class="['w-full text-left px-4 py-2 text-sm hover:bg-(--emerald-surface)', statusFilter === option ? 'font-semibold text-(--emerald-dark)' : 'text-(--emerald-medium)']">
                            {{ option === 'all' ? 'All Status' : option.charAt(0).toUpperCase() + option.slice(1) }}
                        </button>
                    </div>
                </div>
            </div>
            <p class="text-xs text-(--emerald-medium) py-4 font-semibold">Showing {{ filteredContacts.length }} of {{ contacts.length }} contacts</p>
            ...
            <table class="hidden md:table w-full rounded-3xl border border-(--emerald-light)">
                ...
                <tbody>
                    <tr v-for="contact in filteredContacts" :key="contact.email"
                        class="border-b border-(--emerald-light)">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                <div
                                    :class="`${contact.initialsBg} w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm`">
                                    {{ contact.initials }}
                                </div>
